...
def make_histogram(census_col, dp_col, total_col, race_eth):
    ...
    plt.plot(np.log(x_vals),p(np.log(x_vals)),"gray")
    plt.annotate("y = {:.2f}x + {:.2f}\nR^2 = {:.5f}".format(z[0], z[1], rsquare), (min(np.log(x_vals)), min(np.log(y_vals))))
    max_y = max(np.log(y_vals))
    systematic = np.poly1d([0, max_y])
    plt.plot(np.log(x_vals), systematic(np.log(x_vals)),"k")
    plt.annotate("Systematic Errors", (7.5, 3))
    min_x = min(np.log(x_vals))
    random = np.poly1d([-0.5, max_y + 0.5*min_x])
    plt.plot(np.log(x_vals), random(np.log(x_vals)), "k")
    plt.annotate("Random Errors", (8, 0.5))
    plt.xlabel("Log(Number of People)")
    plt.ylabel("Log(Average Absolute % Error)")
    plt.title("% Error in total VAP")
    plt.savefig("avg % error plot log-log avg.png", bbox_inches='tight')
    plt.show()
